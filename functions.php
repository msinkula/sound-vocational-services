...
function get_subpages() { // still working on this.
	
	$page_excerpts = get_children(array('post_type' => 'page','post_status' => null,'post_parent' => $post->ID,'order' => ASC,'orderby' => 'menu_order')); 
	
	if($page_excerpts) {
				
		foreach ($page_excerpts as $post) {
...
function add_flexslider() { 
    ...
    foreach ( $attachments as $attachment_id => $attachment ) { 
		...
		echo '<blockquote >&ldquo;'.$theBlockquote.'&rdquo;</blockquote>';		
		echo '<button class="flexslider-link" href="'.$theLink.'">Learn More&nbsp;&raquo;</button>';
		echo '</div>';
        echo '</li>';
...
function admin_init(){
	...
	function set_staff_member_parent_id() { // set the parent ID for Staff Member profiles
		...
		echo '<input type="text" id="parent_id" name="parent_id" value="70" />'; // creates the value of 2 for the about us section
